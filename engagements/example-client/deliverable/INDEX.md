<!-- upstream: engagements/example-client/INDEX.md -->
---
style: descriptive
---

# deliverable

## Purpose

The agent-executable skill.md the business receives, and known-defects.md beside it. Non-blocking findings from the adversarial reviewer are written down here, not dropped.

## Inventory

Empty. **The interview playbook does not fill this directory either.** `skill.md` is written from the
specification in `../spec/`, using `create-skill`, and then attacked with the
`adversarial-reviewer` persona before it goes anywhere near the owner.

`known-defects.md` sits beside it and holds every non-blocking finding the reviewer raised. Deferring
a defect in writing is delivery; leaving it out is not.

## Freshness

| Item | Last updated | Class | Status |
|---|---|---|---|
| — | 2026-08-11 | Mutable | empty |
