...
CANON="$ROOT/CANON.md"
[[ -s "$CANON" ]] || { echo "FATAL: no CANON.md at $ROOT"; exit 2; }
h(){ shasum -a 256 "$1" 2>/dev/null | cut -c1-64; }
gone=0; moved=0; local_=0; ok=0; unresolved=0
...
echo
echo "in sync: $ok   moved upstream: $moved   gone upstream: $gone   edited locally: $local_"
