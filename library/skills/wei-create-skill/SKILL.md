...
2b. **If user-level and it writes anywhere in a KB, give it a routing table.** Never hardcode a division path in a user-level skill, and never accept the destination as a flag (preference #17 rule 2). Resolve the workspace at runtime:
   ...
   Then `{target-dir}/references/routing.md` maps the resolved root to a destination. A failure here must **stop and report**, never fall back to the current directory. Worked example: `library/skills/wei-digest-doc/references/routing.md`.
...
| Key | Value |
|---|---|
| Skill location (this skill) | `library/skills/wei-create-skill/`, reached through the `.claude/skills` symlink |
...
| Routing worked example | `library/skills/wei-digest-doc/references/routing.md` |
