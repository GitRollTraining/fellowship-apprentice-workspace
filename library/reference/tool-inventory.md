...
| persona | `socratic-tutor` | Non-directive questioning, Answer-directed follow-up | `library/personas/socratic-tutor.md` | Tutors the fellow through Stage 1 training without answering or writing the artifact, so what lands in `training/` is the fellow's own evidence. The only library item that serves training rather than an engagement. |
